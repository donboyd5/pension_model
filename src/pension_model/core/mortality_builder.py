"""
Build mortality tables from raw Excel inputs.

Replaces R's mortality construction (benefit model lines 140-291).
Produces a CompactMortality object from:
  - pub-2010 base mortality tables (3 sheets: General, Teacher, Safety)
  - MP-2018 improvement scale (Male/Female sheets)

The result is a compact (age, year) → rate lookup for employee and retiree
mortality, with class-specific base table selection.

Class → base table mapping:
...
def build_compact_mortality_for_plan(
    constants,
    raw_dir: Path,
    class_name: str = "all",
) -> CompactMortality:
    """Build mortality from plan config's mortality specification.

    Reads the config's mortality section to determine base table and
    improvement scale, then dispatches to appropriate file paths.

    For FRS: falls back to build_compact_mortality_from_excel with FRS paths.
    For TRS: reads PubT-2010(B) + MP-2021 from Inputs/ subdirectory.
    """
    mort_cfg = getattr(constants, "raw", {}).get("mortality", {})
    plan = constants.plan_name if hasattr(constants, "plan_name") else "frs"

    if plan == "frs" or not mort_cfg:
        # FRS: use existing hardcoded path logic
        return build_compact_mortality_from_excel(
    ...
    base_table = mort_cfg.get("base_table", "pub_2010_teacher")
    improvement = mort_cfg.get("improvement_scale", "mp_2021")

    # Resolve base table file + sheet
    inputs_dir = raw_dir / "Inputs"
    base_table_map = {
        "pub_2010_teacher_below_median": (
            inputs_dir / "pub-2010-amount-mort-rates.xlsx", "PubT-2010(B)"),
        "pub_2010_teacher": (
            inputs_dir / "pub-2010-amount-mort-rates.xlsx", "PubT-2010"),
        "pub_2010_safety": (
            inputs_dir / "pub-2010-amount-mort-rates.xlsx", "PubS-2010"),
        "pub_2010_general": (
            inputs_dir / "pub-2010-amount-mort-rates.xlsx", "PubG-2010"),
    }
    improvement_map = {
        "mp_2021": inputs_dir / "mp-2021-rates.xlsx",
        "mp_2018": raw_dir / "mortality-improvement-scale-mp-2018-rates.xlsx",
    }

    if base_table not in base_table_map:
        raise ValueError(f"Unknown base_table: {base_table}")
    if improvement not in improvement_map:
        raise ValueError(f"Unknown improvement_scale: {improvement}")

    pub_path, pub_sheet = base_table_map[base_table]
    mp_path = improvement_map[improvement]
    ...
    max_year = (constants.ranges.start_year + constants.ranges.model_period
                + max_age - min_age)
    base_year = 2010  # anchor year for all pub-2010 tables

    # Read base mortality (same format as headcount tables)
    base = _read_base_mort_table(pub_path, pub_sheet)
