...
| `valuation_inputs.{class}.ben_payment` | `derived` | Current runtime values are exactly `class_outflow * ben_payment_ratio`, where the ratio comes from the ACFR deductions table and the class outflows come from the reviewed legacy baseline | ACFR pension-plan deductions table, printed p. `155`, PDF p. `157`; AV Table C-5, printed p. `C-4`, PDF p. `96`; ACFR statistical `Total Annual Benefits by System/Class`, printed p. `205`, PDF p. `207`; [plans/frs/baselines/input_params.json](/home/donboyd5/Documents/python_projects/pension_model/plans/frs/baselines/input_params.json); [scripts/extract/extract_baseline.R](/home/donboyd5/Documents/python_projects/pension_model/scripts/extract/extract_baseline.R) | Apply `11,944,986,866 / 12,763,932,044` to baseline class outflows | Exact runtime reproduction is now explained. The unresolved issue is upstream provenance of the class outflows: they do not exactly match either the ACFR class annual-benefit table or valuation Table C-5, especially for `special`, `admin`, `eco`, and `eso`. |
...
- the source-to-baseline path for FRS class outflow inputs used in `ben_payment`
